...
class Cpa {
	 ...
	 * Keep this field empty, if don't care about security of your CPA :)
	 */
	...
	/**
		...
		/**
		 * Decimal point, needed for CPA amount calculations
		 * (don't change this value, because it may cause loss of your CPA)
		 */
		const DECIMAL_POINT = 8;

		/**
		 * Number of decimal places
		 * (you may change this value, if price of CPA goes to high one day)
		 */
		const PREC_POINT = 4;
		...
		public function __construct($rpc_host = "", $rpc_port = "", $rpc_ssl = false) {
			...
			$this->service_mixin = self::CPA_MIXIN;
			$this->service_fee = self::CPA_FEE;
...
echo "</br></br>data</br>".var_dump($data)."</br></br></br>";
		 ...
		 *
		 * @param string [$address] Addres of Cpa wallet
		 */
		public static function checkAddress($address) {

			$result = preg_match('/^cp[123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz]{95}$/', $address);
